...
Render.imgs = (list) => {
	return _.go(`
		<div class="image">
			<div class="box">
				<img src="${list.url}" class="fade" data-lazy="${list.url}">
			</div>
...
Render.confirm = () => new Promise( resolve => _.go(`
		<div class="confirm">
			<div class="body">
		...
		(res)=>$.$findAll('button',res),
		_.each(btn => _.tap(
			$.$on('click', (e)=>_.go(e.currentTarget,
				(v)=>{
					$.$remove($.$closest('.confirm', v));
					resolve(v.value);
...
! async function imageF() {
	const imgs = await Images.fetch();
	const imgListWrap = document.querySelector('.img-list-wrap');
	_.go(
		imgs,
		L.map(Render.imgs),
		L.each(html => $.$appendTo(imgListWrap, html)),
		// delete click event
		(img)=>_.go(img,
			_.each(img=>{
				$.$on("click", async ({currentTarget})=> {
					const isRemove = await Render.confirm();
					if(isRemove === 'true') $.$remove($.$closest('.image', currentTarget));
				})(img)
			}),
		),
		...
		(res)=>_.tap(
			log(C.takeAll(res.then(res=>res)))
		),
		_.each(async (img)=>{
			img.src = img.getAttribute('data-lazy')
			await $.$addClass('fade-in', img);
		})
	)	
...
// tap => 안에서 어떤 변화를 주어도 그후에 값에 변화를 주지않는다
